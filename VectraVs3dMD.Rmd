...
```{r}
#Create new dataframe to store the results per landmark
md.35ind.mapping.euclid.tem <- matrix(nrow = 7160,ncol=3,dimnames=list(NULL,c("TEM", "rTEM", "R")))
...
#For each landmark, arrange the three mappings of each image in columns and images in rows, then calculate the TEM and store it, then calculate relative TEM and coefficient of reliability
for (i in 1:7160){
  tmp <- md.35ind.mapping.euclid[c(i,7161:7163)]
  M <- tmp %>% spread(Mapping, colnames(tmp)[1]) %>% select(c("M1", "M2", "M3"))
  md.35ind.mapping.euclid.tem[i,1] <- tem(M)
  md.35ind.mapping.euclid.tem[i,2] <- (tem(M)/mean(as.matrix(M)))*100
  md.35ind.mapping.euclid.tem[i,3] <- 1-(tem(M)^2/sd(as.matrix(M))^2)
}
```
